"""
Pytest configuration and fixtures for arXiv messaging service tests
"""
...
from src.message_server import (
    EventAggregationSystem, UserPreference, DeliveryMethod, 
    AggregationFrequency, AggregationMethod, Event, EventStore, EventType
)
...
@pytest.fixture
def mock_subscription_name():
    """Mock Pub/Sub subscription name"""
...
def mock_firestore_client():
    ...
    with patch('src.message_server.firestore.Client') as mock_client:
...
def mock_pubsub_client():
    """Mock Pub/Sub client"""
    with patch('src.message_server.pubsub_v1.SubscriberClient') as mock_client:
        yield mock_client.return_value


@pytest.fixture
def mock_event_store(mock_firestore_client):
    """Mock EventStore with Firestore client"""
    with patch.dict('os.environ', {'GCP_PROJECT_ID': 'test-project'}):
        store = EventStore()
        store.db = mock_firestore_client
        yield store


@pytest.fixture
def event_aggregation_system(mock_project_id, mock_subscription_name):
    """EventAggregationSystem instance for testing"""
    with patch('src.message_server.EventStore') as mock_event_store_class, \
         patch('src.message_server.PubSubEventProcessor') as mock_pubsub_processor:
